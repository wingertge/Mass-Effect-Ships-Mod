...
public class ItemLaunchPadController extends ItemOpenBlock {
    public static final String ENERGY_STORAGE_TAG = "EnergyStorage";

    public ItemLaunchPadController(Block block) {
        super(block);
    }

    private static void addEnergyStorageInfo(NBTTagCompound tag, String name, String format, List<String> result) {
        if (tag.hasKey(name)) {
            SyncableEnergyStorage storage = new SyncableEnergyStorage(Config.EnergyStorage.LAUNCH_PAD_CONTROLLER);
            storage.readFromNBT(tag, name);
            result.add(StatCollector.translateToLocalFormatted(format, storage.getEnergyStored(), Config.EnergyStorage.LAUNCH_PAD_CONTROLLER));
        }
    }
